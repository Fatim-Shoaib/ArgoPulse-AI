...
def load_config():
    ### CHANGED: Added defaults for new data file settings ###
    default_config = {
...
# --- LLM-Powered Filtering Function ---
...
def main():
    ...
    try:
        ...
        consolidated_data = []
        ### NEW: Get data file settings from config ###
        ...
        for path in sorted(final_paths):
            if path.is_file():
                ...
                ### CHANGED: Added logic for handling data files ###
                if path.name == '.env':
                    content_data['content'] = "[.env content is hidden]"
                elif path.suffix in data_file_ext:
                    try:
                        with open(path, 'r', encoding='utf-8') as f:
                            head_lines = [next(f, '') for _ in range(head_lines_count)]
                            preview_content = "".join(head_lines)
                        content_data['content'] = f"[Preview of data file (first {head_lines_count} lines)]:\n{preview_content}"
                        logger.debug(f"Extracted head from data file: {path.name}")
                    except Exception as e:
                        content_data['content'] = f"[Error reading preview of data file: {e}]"
                else:
                    try:
                        with open(path, 'r', encoding='utf-8') as f:
